...
import tensorflow.keras.layers as KE # Engine functionality moved to layers
...
class DataGenerator(KU.Sequence):
    ...
    def __len__(self):
        l = int(np.ceil(len(self.image_ids) / float(self.batch_size)))
        return l
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            """
                In Keras, there was no layer corresponding to a constant layer, so 
                there was a shortcut to using a lambda layer. It doesn't work anymore!
                By the way, there is an input function. Why is it woven like that? 
                Anyway, I decided to write a custom layer and return a constant tensor.
             """
